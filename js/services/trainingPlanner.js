...
export function generatePersonalizedPlan(profile) {
  ...
  const activeDays = dayNames.slice(0, Math.min(trainingDays, 6));
  if (trainingDays >= 7 || !activeDays.includes('sunday')) {
    // keep sunday as recovery if training 6+ days
  }
...
function blendCategories(base, priority, weight) {
  ...
  base.forEach(c => {
    if (!blended.includes(c)) blended.push(c);
  });
...
export function getTodaySession(plan) {
  ...
  if (!plan?.plan?.[day]) {
    return generateFallbackSession(day);
  }
  return { day, ...plan.plan[day] };
}

function generateFallbackSession(day) {
...
export function generateEvaluation(profile, progress) {
  ...
  if (profile.improvementAreas?.includes('finishing')) {
    improvements.push('Continue focusing on finishing under pressure');
  }
  if (profile.improvementAreas?.includes('weak foot')) {
    improvements.push('Dedicate more sessions to weak foot development');
  }
